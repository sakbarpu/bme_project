...
Notice that this is merely a toy implementation and
not meant for large scale deployment.

The implementation is supposed to work for small corpus. 
It takes as input the corpus file and the word2vec 
parameters file, and returns word embeddings saved as 
numpy array file to an specified output path.
...
implement SG HS 
implement CBOW HS
implement multithreading to make it fast
may be get rid of pandas
replace term based access to W and Z to integer based access
...
class Word2Vec:
	...
	def init_model(self):
		...
		#self.W = np.random.rand(self.size,len(self.vocab)) #from input to proj
		#self.Z = np.zeros((self.size,len(self.vocab)),dtype=float) #from proj to output
		#self.W = pd.DataFrame(self.W, columns=sorted(self.vocab.keys())) #make a dataframe W
		#self.Z = pd.DataFrame(self.Z, columns=sorted(self.vocab.keys())) #make a dataframe Z
	
	'''
	def get_random_train_sample(self, start=None, stop=None, counter_worker=None):
		#Out of all the sentences/documents in the corpus (that are separated by line break in corpus file)
		#get a random portion of text out.
		
		#First get a random sentence out.
		#Then in that sentence get a random position out. 
		#Then gather all the words around that position in the sentence and return.
		
		#Return format is as: [[list of context words] , focus word]

		if start == None: start = 0
		if stop == None: stop = len(self.sentences)
		
		while True:
			random_sent_number = np.random.randint(start,stop) #get a random sentence
			sent = self.sentences[random_sent_number]
			#print (sent)
			if sent is None: print ('None')
			return
			if sent is None: 
				continue
			else:
				sent = sent.strip().split(" ") #split sentence to form words
				sent = [x.strip() for x in sent] #remove any additional line breaks etc from each word in sentence
				random_position_sent = np.random.randint(0,len(sent)) #get a random positon in the sentence
				
				#if random_position_sent - self.window < 0: continue #check for boundary
				#if random_position_sent + self.window > len(sent): continue
				
				return [sent[random_position_sent-self.window : random_position_sent] +
						sent[random_position_sent+1: random_position_sent+self.window],
						sent[random_position_sent]]
	'''
	...
	def call_a_sgwithns_thread(self, local_sents, start, stop, counter_worker):
		...
		for local_iter in range(self.iters):
			start_time = time.time()
			for sent in local_sents[start,stop]:
				...
				for counter_terms in range(len(sent)): #loop over the sentence the length of sentence times
					...
					for context_word in train_sample[0]:
						...
						if context_word not in self.W: continue
			...
			print ("WORKER:", counter_worker, "DONE WITH ITER:", local_iter, "Took time:", time.time()-start_time)	
		print ("WORKER:", counter_worker, "DONE Training,", "Trained", local_num_words_processed, "examples")
		
		'''		
		while True:

			if (local_num_words_processed + 1) % self.batch_words == 0:
				local_batch_count += 1

				#update current alpha 
				self.alpha = self.start_alpha * (1.0 - local_num_words_processed / float(self.iters * self.total_words_in_corpus + 1))
				if self.alpha < self.start_alpha * 0.0001: self.alpha = self.start_alpha * 0.0001
				print("WORKER:", counter_worker, "iter: ", self.current_num_iters+1, ", word count: ", local_num_words_processed, 
						", alpha: ", self.alpha, ", batches:", local_batch_count) #print message
				return
			if self.current_num_words_processed >= self.total_words_in_corpus:
				self.current_num_iters += 1
				if self.current_num_iters >= self.iters: break
			
			train_sample = self.get_random_train_sample(start, stop, counter_worker) #get a training example
			local_num_words_processed += 1
			self.current_num_words_processed += 1
			#print (counter_worker, self.current_num_words_processed)
		
			#loop over each context word in the window of training sample and train on (word, context) pair
			for context_word in train_sample[0]:
				if context_word not in self.W: continue #skip words not in vocab
				if train_sample[1] not in self.W: continue 
				...
				for d in range(0, self.negative):
					if d == 0: 
						...
						context_word = self.sorted_vocab_words[self.unigram_table.searchsorted(np.random.randint(self.unigram_table[-1]))]
						label = 0
					f = np.dot(self.W[train_sample[1]],self.Z[context_word]) #propagate proj to output
				...
				self.W[train_sample[1]] = self.W[train_sample[1]] + neu # learn input to proj 
				#print ("To do one update on W and Z it takes ", time.time() - start, "sec")
			'''

	def train_sg_model_with_ns_1(self):
	...
	def train_sg_model_with_ns(self):
		...
		train_word_count = 0 #how many words we have trained so far
		start_alpha = self.alpha #the alpha parameter in the sgd we are starting with
		curr_batch_count = 0 #how many batches we have trained so far
		labels = np.zeros(self.negative+1)
		labels[0] = 1.0
		for it in range(0,self.iters):
			
			for counter_examples in range(0,self.total_words_in_corpus):
				if counter_examples%1000==0: print (counter_examples, "/", self.total_words_in_corpus)
				#if timing == 1: start= time.time()
				train_sample = self.get_random_train_sample() #get a training example
				 
				train_word_count += 1
				v_w = self.W[train_sample[1]]
				...
				for context_word in train_sample[0]:
					...
					words = [context_word]
					for d in range(0, self.negative):
						words.append(self.sorted_vocab_words[self.unigram_table.searchsorted(np.random.randint(self.unigram_table[-1]))])
					v_p_ws = np.array([self.Z[word] for word in words]).T
					f = np.dot(v_w, v_p_ws) # propagate proj to output
					output = expit(f) # logistic/sigmoid function
					g = (labels - output) * self.alpha # compute gradient
					tmp = np.outer(g, v_w).T
					for c in range(len(words)): self.Z[words[c]] += tmp[:,c] # learn proj to output
					err_sum += np.dot(g, v_p_ws.T) # update error sum
					print (time.time() - start)
				self.W[train_sample[1]] += err_sum # learn input to proj
				
				if train_word_count == self.total_words_in_corpus or train_word_count % self.batch_words == 0.0:
					curr_batch_count += 1
					self.alpha = start_alpha * (1.0 - train_word_count / float(self.iters * self.total_words_in_corpus + 1))
					if (self.alpha < start_alpha * 0.0001): self.alpha = start_alpha * 0.0001;
					print("iter: ", it, ", word count: ", train_word_count, ", alpha: ", self.alpha, 
							", batches:", curr_batch_count) #print message
				#if timing == 1: print ("time:", time.time()-start)
			if timing==1: print ("Iteration:", it, "finished")
 
	def train_sg_model_with_ns_2(self):
		...
		while True:
			...
			if train_word_count % self.batch_words == 0: #have we trained a batch completely? 
				curr_batch_count += 1 #increase batch count
				
				#update current alpha (alpha is updated after every batch)
				self.alpha = start_alpha * (1.0 - train_word_count / float(self.iters * self.total_words_in_corpus + 1))
				if (self.alpha < start_alpha * 0.0001): self.alpha = start_alpha * 0.0001;
				
				print("iter: ", self.current_num_iters+1, ", word count: ", train_word_count, ", alpha: ", self.alpha, 
						", batches:", curr_batch_count) #print message

			if train_word_count >= self.total_words_in_corpus: #have we trained on number of words equal total size of corpus?
				self.current_num_iters += 1 #increase iteration count 
				if self.current_num_iters >= self.iters: break #end training if trained for iters number of iterations
	...
	def save(self, out_path):
		...
		self.W.to_pickle(out_path + '/input.vectors')
		self.Z.to_pickle(out_path + '/output.vectors')
...
def main(argv):
	...
	t0 = time.time()
	#model = Word2Vec(sentences=sentences, min_count=0, size=500, sg=1, hs=0, negative=15, iters=2,
	#				 window=8, compute_loss=True, workers=16)
	model = Word2Vec(contentpath=processedfilepath, min_count=0, size=500, sg=1, hs=0, negative=15, iters=10,
					 window=8, compute_loss=True, workers=16)
	model.build_model()
	t1 = time.time()
